...
const (
	...
	getTaskById     = `SELECT id, title, description, status, created_at, updated_at FROM tasks WHERE id = $1;`
)
...
func (r *repository) GetTaskById(ctx context.Context, id int) (*Task, error) {
	var task Task
	err := r.pool.QueryRow(ctx, getTaskById, id).Scan(&task.ID, &task.Title, &task.Description, &task.Status, &task.CreatedAt, &task.UpdatedAt)
	if err != nil {
